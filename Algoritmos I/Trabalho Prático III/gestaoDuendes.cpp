...
void GestaoDuendes::InsereConflito(int i_duende, int j_duende){
    ...
    grafo_duendes[i_duende].push_back(j_duende);
    grafo_duendes[j_duende].push_back(i_duende);
}
...
std::vector<int> GestaoDuendes::GrupoComDuende(const std::vector<int>& grupo_atual, int i_duende){
    ...
    for (int x : grupo_atual) {
        if (x == i_duende) continue; 

        //Verifica conflito
        bool conflito_i = false;
        for (int adj : grafo_duendes[i_duende]) { 
            if (adj == x) {
                conflito_i = true;
                break;
            }
        ...
        //Se não conflita adiciona ao conjunto
        if (!conflito_i) {
            grupo_com_i.push_back(x); 
        }
    }
...
std::vector<int> GestaoDuendes::ArvoreDecisao(const std::vector<int>& grupo_atual, std::map<std::string, std::vector<int>>& registro_solucoes){
    ...
    //Define ID para grupo de elementos a ser encontrado o melhor agrupamento
    ...
    std::sort(solu_com_i.begin(), solu_com_i.end());
    ...
    if(grupo_sem_i.size() > grupo_com_i.size()){
        ...
        //Se é igual verifica qual é mlehro lexigroficamente
        ...
        if(menor_lex == 2){         maior_grupo = grupo_com_i;  }
        else if(menor_lex == 1){    maior_grupo = grupo_sem_i;  }
    ...
    for (const auto& solucao_A : todas_solucoes_A) {
        ...
        for (int duende_b : metade_B) {
            ...
            for (int duende_a : solucao_A) {
                ...
                for (int adj : grafo_duendes[duende_a]) { 
                    if (adj == duende_b) {
                        conflito = true;
                        break; 
                    }
...
void GestaoDuendes::GeraGrupo(const std::vector<int>& conjunto,int i_duende, std::vector<int>& subconjunto_atual,std::vector<std::vector<int>>& grupos) {
    ...
    for (int duende : subconjunto_atual) {
        ...
        for (int vizinho : grafo_duendes[duende_atual]) {
